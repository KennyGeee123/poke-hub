...
function HpBar({ cur, max }: { cur: number; max: number }) {
  ...
  return (
    <div>
      <div
        style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 2 }}
      >
        <span style={{ color: "var(--t2)" }}>HP</span>
        <span style={{ fontFamily: "Bebas Neue", letterSpacing: 1, color: col, fontSize: 13 }}>
          {cur}/{max}
        </span>
      </div>
      <div
...
function MiniCard({
  ...
  bc: { card: TCGCard; hp: number; maxHp: number } | null;
  size?: number;
  hl?: boolean;
  withSprite?: boolean;
  flip?: boolean;
...
function BenchRow({
  ...
  bench: { card: TCGCard; hp: number; maxHp: number }[];
  size?: number;
    ...
    <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
      {Array.from({ length: slots }).map((_, i) => {
        ...
        return (
          <div
            key={i}
            onClick={() => bc && onSelect?.(i)}
            style={{ cursor: bc && onSelect ? "pointer" : "default" }}
          >
  ...
  const basics: TCGCard[] = [
    {
      ...
      attacks: [
        { name: "Fire Spin", damage: "100", cost: ["Fire", "Fire", "Fire", "Fire"], text: "" },
      ],
      tcgplayer: { prices: { holofoil: { market: 399.99 } } },
    },
    {
      id: "pv-guest-blastoise",
      name: "Blastoise",
      supertype: "Pokémon",
      subtypes: ["Stage 2"],
      hp: "100",
    ...
    },
    {
      ...
      attacks: [
        { name: "Solarbeam", damage: "60", cost: ["Grass", "Grass", "Grass", "Grass"], text: "" },
      ],
...
function ensureDeckWithBasics(cards: TCGCard[]): TCGCard[] {
  ...
  if (playable.length >= 6)
    return cards.length >= 6 ? cards : [...cards, ...guestStarterDeck()].slice(0, 30);
...
async function fetchRandomDeck(era: Era | undefined, size = 24): Promise<TCGCard[]> {
  ...
  const pkmQueries = [
    eraQ
      ? `${eraQ} subtypes:basic supertype:Pokémon hp:[60 TO *] (${typeQ})`
      : `subtypes:basic supertype:Pokémon hp:[60 TO *] (${typeQ})`,
    eraQ
      ? `${eraQ} subtypes:basic supertype:Pokémon (${typeQ})`
      : `subtypes:basic supertype:Pokémon (${typeQ})`,
...
export function BattleView({
  ...
  useEffect(() => {
    ...
    const fromVault = Object.values(vault)
      .flatMap((e) => Array(Math.min(e.qty, 4)).fill(e.card))
      .filter(isPlayablePokemon) as TCGCard[];
    setLoadingDeck(true);
    Promise.all([fetchRandomDeck(era, 24), fetchRandomDeck(era, 24)])
      .then(([playerPool, aiPool]) => {
        if (cancelled) return;
        const playerDeck = ensureDeckWithBasics(
          shuffle([...fromVault, ...playerPool]).slice(0, 30),
        );
        setDeck(playerDeck);
  ...
  useEffect(() => {
    ...
    const aiSrc = aiDeckOverride && aiDeckOverride.length >= 6 ? aiDeckOverride : deck;
      ...
      setGs((prev) => {
        ...
        if (g.phase !== "gameOver") {
          ...
          if (g.player.deck.length > 0) {
            g.player.hand.push(g.player.deck.shift()!);
            g.log.unshift("▶ You draw a card.");
          }
        }
        return g;
      });
  ...
  if (!deck || loadingDeck) {
    return (
      <div className="pad">
        <button className="pv-back" onClick={onExit}>
          ← Back
        </button>
    ...
    return (
      <div className="pad">
        ...
        <div className="pv-empty">
          ...
          <div>
            Couldn’t build a starter deck. Check your connection and try Quick Battle again.
          </div>
        </div>
      </div>
    );
  ...
  const canAttack =
    isTurn && !gs.player.attacked && !!gs.player.active && !!gs.ai.active && !aiThink;
  ...
  function doAttack(atkIdx: number) {
    setGs((prev) => {
      ...
      const g = applyAttack(prev, "player", atkIdx);
      if (g.phase !== "gameOver") {
        g.phase = "aiTurn";
        g.log.unshift("🤖 AI's turn…");
      }
      return g;
    });
  }
  ...
  return (
    <div className="pad pv-battle">
      ...
      >
        ...
        <div
          style={{ fontFamily: "Bebas Neue", letterSpacing: 3, fontSize: 18, color: "var(--gold)" }}
        >
          ⚡ ARENA
        </div>
      ...
      <div className={`pv-board pv-arena${attackFlash ? ` flash-${attackFlash}` : ""}`}>
          ...
          <div className="pv-victory">
            ...
            <h2
              style={{
                ...
                letterSpacing: 4,
                fontSize: 36,
                color: gs.winner === "player" ? "var(--gold)" : "#f87171",
              }}
            >
              {gs.winner === "player" ? "VICTORY" : "DEFEAT"}
            </h2>
            <button className="pv-btn pv-btn-fill yes" style={{ marginTop: 18 }} onClick={onExit}>
              Back to Vault
            </button>
          </div>
        )}
        ...
        <div className="pv-hud">
          <HudSide
            label="🤖 OPPONENT"
            color="#f87171"
            prizes={gs.ai.prizes.length}
            deck={gs.ai.deck.length}
            hand={gs.ai.hand.length}
          />
          ...
          <HudSide
            label="▶ YOU"
            color="#38bdf8"
            prizes={gs.player.prizes.length}
            deck={gs.player.deck.length}
            hand={gs.player.hand.length}
            right
          />
        </div>
        ...
        <div className="pv-zone">
          ...
          >
            <div
              className="pv-active-wrap ai"
              style={
                {
                  position: "relative",
                  "--aura": tc(gs.ai.active?.card.types?.[0] ?? "Colorless"),
            ...
            >
              <MiniCard
                ...
                size={120}
                withSprite
                attacking={attackFlash === "ai"}
                hit={attackFlash === "player"}
                side="ai"
              />
        ...
        <div className="pv-zone">
          ...
          >
            <div
              className="pv-active-wrap me"
              style={
                {
                  position: "relative",
                  "--aura": tc(gs.player.active?.card.types?.[0] ?? "Colorless"),
                } as React.CSSProperties
              }
            >
              <MiniCard
                bc={gs.player.active}
                size={120}
                hl={isSetup && !gs.player.active}
                withSprite
                flip
                attacking={attackFlash === "player"}
                hit={attackFlash === "ai"}
                side="player"
              />
            ...
            {dmgFloat && attackFlash === "ai" && (
              <div className="pv-dmgfloat" style={{ left: "auto", right: "60%" }}>
                {dmgFloat}
              </div>
            )}
            <div style={{ minWidth: 180, display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {isTurn && (
                <button className="pv-btn pv-btn-out" onClick={endTurn} disabled={aiThink}>
                  End Turn →
                </button>
              )}
              {isSetup && (
                <div style={{ fontSize: 11, color: "var(--t2)" }}>↓ Tap a Pokémon in your hand</div>
              )}
            </div>
          </div>
          <BenchRow
            ...
            size={50}
            onSelect={(i) => !gs.player.active && promote(i)}
          />
        </div>

        {/* Hand */}
        <div className="pv-hand">
          <div style={{ fontSize: 10, color: "var(--t2)", marginRight: 6, whiteSpace: "nowrap" }}>
            HAND ({gs.player.hand.length})
          </div>
...
function HudSide({
  ...
  return (
    <div style={{ textAlign: right ? "right" : "left" }}>
      <div
        style={{ fontFamily: "Bebas Neue", letterSpacing: 2, fontSize: 12, color, marginBottom: 4 }}
      >
        {label}
      </div>
      <div
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            ...
            style={{
              ...
              height: 9,
              borderRadius: "50%",
              background: i < prizes ? "var(--gold)" : "rgba(255,255,255,.08)",
              border: "1px solid rgba(255,255,255,.12)",
            }}
          />
        ))}
        <span style={{ fontSize: 9, color: "var(--t2)", marginLeft: 4 }}>
          {prizes}p · {deck}d · {hand}h
        </span>
      </div>
    </div>
  );
...
export function BattleHub({ onExit }: { onExit: () => void }) {
  ...
  if (mode === "battle" && customDeck)
    return <BattleView customDeck={customDeck} era={era} onExit={() => setMode("hub")} />;
  if (mode === "deck")
    return (
      <DeckBuilder
        onBack={() => setMode("hub")}
    ...
    <div className="pad pv-hub">
      ...
      <div className="pv-hub-banner">
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          ...
          <p style={{ color: "var(--t2)", fontSize: 13, maxWidth: 460 }}>
            Duel a tactical AI using real Pokémon, Trainer & Energy cards across every era of the
            TCG — live from the Pokémon TCG API.
          </p>
          <button
      ...
      <div className="pv-hub-grid" style={{ marginTop: 18 }}>
        <button className="pv-hub-card primary" onClick={() => setMode("quick")}>
          ...
          <div className="pv-hub-sub">
            Random {era.name} deck — Pokémon, Trainers & Energy — straight into a duel.
          </div>
        ...
        <button className="pv-hub-card" onClick={() => setMode("deck")}>
          ...
          <div className="pv-hub-sub">
            Hand-pick cards from your Vault or the API. Up to 4 copies per card, 4–60 total.
          </div>
    ...
    <div className="pv-modal-bg" onClick={onClose}>
      <div className="pv-modal" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          <button className="pv-btn pv-btn-out" onClick={onClose}>
            ✕ Close
          </button>
        </div>
        ...
        <ul>
          <li>
            Take all <b>6 Prize cards</b> by Knocking Out opponent Pokémon.
          </li>
        ...
        <ol>
          ...
          <li>
            Shuffle your 60-card deck. Draw <b>7 cards</b>.
          </li>
          <li>
            Place 1 <b>Basic Pokémon</b> face-down as your Active. Up to 5 Basics on your{" "}
            <b>Bench</b>.
          </li>
          <li>
            Place 6 cards from the top of your deck face-down as <b>Prize cards</b>.
          </li>
          <li>Flip Active and Bench face-up — battle begins!</li>
          <li>
            No Basics in hand? Reveal your hand, shuffle, redraw — opponent draws an extra card.
          </li>
        </ol>
        ...
        <ol>
          <li>
            <b>Draw 1 card</b> from your deck.
          </li>
          <li>
            ...
            <ul>
              ...
              <li>
                Attach <b>1 Energy</b> from your hand to one of your Pokémon (once per turn).
              </li>
              <li>
                Play any number of <b>Item</b> Trainer cards.
              </li>
              <li>
                Play <b>1 Supporter</b> card per turn.
              </li>
              <li>
                Play <b>1 Stadium</b> card per turn (replaces any in play).
              </li>
              <li>
                Use Pokémon <b>Abilities</b>.
              </li>
              <li>Retreat your Active (pay the retreat cost in Energy — once per turn).</li>
            </ul>
          </li>
          <li>
            <b>Attack</b> — your Active uses an attack if it has the required Energy. This ends your
            turn.
          </li>
        </ol>
        ...
        <ul>
          <li>
            Damage = attack damage, doubled by <b>Weakness</b>, reduced 30 by <b>Resistance</b>.
          </li>
        ...
        <ul>
          <li>
            <b style={{ color: "#38bdf8" }}>Pokémon</b> — your fighters. Basics can be played
            directly; Stage 1/2 evolve from a previous stage.
          </li>
          <li>
            <b style={{ color: "#a855f7" }}>Trainer</b> — Items, Supporters, Stadiums, Tools.
            Powerful one-shot effects.
          </li>
          <li>
            <b style={{ color: "#eab308" }}>Energy</b> — fuel for attacks. One attach per turn;
            Special Energy give bonuses.
          </li>
        </ul>

        <h3 className="pv-rule-h">⚙️ This App's Implementation</h3>
        <p>
          Pokévault's battle engine simulates the core loop: Basics, Bench (5), Prizes (6), KO →
          prize, weakness ×2, resistance −30, and Trainer / Energy plays with simplified one-shot
          effects. Evolution chains, Special Energy, retreat costs, status conditions, and Stadium
          slots are omitted for speed of play.
        </p>

        <p style={{ marginTop: 14, fontSize: 11, color: "var(--t3)" }}>
          Full rulebook PDFs:{" "}
          <a
            href="https://www.pokemon.com/us/pokemon-tcg/play-pokemon-tcg/"
            target="_blank"
          ...
          ·{" "}
          <a
            href="https://bulbapedia.bulbagarden.net/wiki/Pok%C3%A9mon_Trading_Card_Game"
            target="_blank"
            rel="noopener noreferrer"
            style={{ color: "var(--gold)" }}
          >
...
function DeckBuilder({
  ...
  const vaultPokemon = Object.values(vault)
    .filter((e) => isPlayablePokemon(e.card))
    .map((e) => e.card);
  ...
  return (
    <div className="pad pv-dbuild">
      ...
      >
        <button className="pv-back" onClick={onBack} style={{ marginBottom: 0 }}>
          ← Hub
        </button>
        <div
          style={{ fontFamily: "Bebas Neue", letterSpacing: 3, fontSize: 22, color: "var(--gold)" }}
        >
          🃏 DECK BUILDER
        </div>
        <button
      ...
      <div className="pv-dbuild-grid">
        <div>
          <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
            <button
              className={`pv-pill ${tab === "vault" ? "on" : ""}`}
              onClick={() => setTab("vault")}
            >
              📦 Vault ({vaultPokemon.length})
            </button>
            <button
              className={`pv-pill ${tab === "search" ? "on" : ""}`}
          ...
          {loading ? (
            ...
            <div className="pv-empty">
              ...
              <div className="pv-empty-title">
                {tab === "vault" ? "NO POKÉMON IN VAULT" : "SEARCH TO FIND CARDS"}
              </div>
            </div>
          ) : (
            <div className="pv-dbuild-pool">
              {pool.map((card) => {
                ...
                return (
                  <div
                    key={card.id}
                    className={`pv-dbuild-card ${inDeck ? "in" : ""}`}
                    onClick={() => add(card)}
                  >
                    <img {...hdImg(card)} alt={card.name} loading="lazy" />
                    <div style={{ padding: 6 }}>
        ...
        <div className="pv-dbuild-panel">
          ...
          >
            <div>
              <div style={{ fontFamily: "Bebas Neue", letterSpacing: 2, fontSize: 14 }}>
                YOUR DECK
              </div>
              ...
              Object.values(deck).map((e) => (
                <div key={e.card.id} className="pv-dbuild-row">
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <button className="pv-qty" onClick={() => remove(e.card.id)}>
                      −
                    </button>
                    <span
                      style={{
                    ...
                    <button className="pv-qty" onClick={() => add(e.card)} disabled={e.qty >= 4}>
                      ＋
                    </button>
                  </div>
                </div>
