...
import { searchCards, getSets, type TCGCard, type TCGSet } from "@/lib/pokemon-api";
import {
  addToParty,
  fetchParty,
  gainXP,
...
export function GameBoyView() {
  ...
  async function loadSet(s: TCGSet) {
    ...
    setSetsLoading(true);
    try {
      const res = await searchCards({ q: `set.id:${s.id} supertype:Pokémon`, pageSize: 60 });
  ...
  useEffect(() => {
    const onWild = async (e: Event) => {
      ...
      const mon = party[0];
      if (!mon) {
        alert("Pick a starter in the Game Boy first!");
        setScene("starter");
        return;
      }
      try {
  ...
  async function tryCatch() {
    ...
    if (Math.random() < chance) {
      ...
      // Add foe as a TCG-like card stub: rebuild via searchCards
      try {
    ...
    <div className="gb-wrap">
      <div className="gb-frame">
        <div className="gb-top">
          ...
          <div className="gb-brand">POKÉVAULT · DOT MATRIX WITH STEREO SOUND</div>
          <div className="gb-dot-cluster"><span /><span /><span /></div>
        </div>
        <div className="gb-screen">
          <div className="gb-scanlines" aria-hidden />
          {scene === "menu" && (
            <Menu party={party} onParty={() => setScene("party")} onStarter={() => setScene("starter")} />
          )}
          {scene === "party" && (
            <PartyView
              party={party}
...
function Menu({ party, onParty, onStarter }: { party: GBMon[]; onParty: () => void; onStarter: () => void }) {
  return (
    <div className="gb-page">
      <div className="gb-title">▶ POKÉVAULT ◀</div>
      <div className="gb-sub">RPG MODE</div>
      <div className="gb-menu">
        <button className="gb-mi" onClick={onParty} disabled={!party.length}>
          {party.length ? `CONTINUE  (${party.length})` : "NO PARTY"}
        </button>
        <button className="gb-mi" onClick={onStarter}>CHOOSE A POKÉMON</button>
      </div>
        ...
        {party.map((m) => (
          <div className="gb-pm" key={m.id}>
            ...
            <div className="gb-pm-info">
              ...
              <div className="gb-pm-meta">Lv {m.level} · HP {m.max_hp} · {m.wins}W/{m.losses}L</div>
              <div className="gb-xpbar"><span style={{ width: `${Math.min(100, (m.xp / xpForNext(m.level)) * 100)}%` }} /></div>
            </div>
            <div className="gb-pm-acts">
              <button className="gb-mi gb-mi-sm" onClick={() => onBattle(m)}>BATTLE</button>
      ...
      <div className="gb-page">
        ...
        <div className="gb-list">
          {sets.slice(0, 80).map((s) => (
            <button key={s.id} className="gb-mi gb-mi-sm" onClick={() => onSet(s)}>
              {s.name.toUpperCase()} <span style={{ opacity: 0.7 }}>· {s.releaseDate?.slice(0, 4)}</span>
            </button>
          ))}
        </div>
        <button className="gb-mi gb-mi-sm" onClick={onBack}>BACK</button>
...
function BattleScreen({
  me, meHp, foe, foeHp, meFx, foeFx, log, busy, pane, party, potions,
    ...
    <div className="gb-page gb-battle">
      ...
      {pane === "main" && (
        <div className="gb-moves">
          ...
          <button className="gb-move" disabled={busy || potions <= 0} onClick={() => onPane("bag")}>🎒 BAG ({potions})</button>
          <button className="gb-move" disabled={busy || party.length < 2} onClick={() => onPane("switch")}>↺ PKMN</button>
      ...
      {pane === "fight" && (
        <div className="gb-moves">
          {me.attacks.slice(0, 4).map((m, i) => (
            <button key={i} className="gb-move" disabled={busy} onClick={() => onMove(m)}>
      ...
      {pane === "bag" && (
        <div className="gb-moves">
          <button className="gb-move" disabled={busy || potions <= 0} onClick={onPotion}>
            ✚ POTION <span>·30HP</span>
          </button>
          <button className="gb-move" disabled={busy} onClick={onCatch}>
            ⚪ POKÉ BALL
          </button>
          <button className="gb-move gb-move-run" disabled={busy} onClick={() => onPane("main")}>BACK</button>
        </div>
