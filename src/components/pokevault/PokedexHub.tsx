...
export function PokedexHub() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/pokemon?limit=2000`)
      ...
      .then((d: any) => {
        if (mounted) setAll(d.results ?? []);
      })
      .catch(() => {
        if (mounted) setListErr("PokéAPI didn’t load. Check your connection and retry.");
      });
    return () => {
      mounted = false;
    };
  ...
  useEffect(() => {
    ...
    j<{ pokemon: { pokemon: { name: string } }[] }>(`${BASE}/type/${type}`)
      .then((d) => {
        if (mounted) setTypeNames(d.pokemon.map((p) => p.pokemon.name));
      })
      .catch(() => {
        if (mounted) setTypeNames([]);
      });
    return () => {
      mounted = false;
    };
  ...
  const loadDetail = useCallback(async (name: string) => {
    ...
    try {
      ...
      try {
        species = await j(p.species.url, ctrl.signal);
      } catch {
        /* ignore */
      }
      const flavor =
    ...
    <div className="px-4 py-4 max-w-6xl mx-auto">
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold tracking-wider" style={{ color: "var(--gold)" }}>
            POKÉDEX HUB
          </h2>
        </div>
        <input
      ...
      {!selected && !listErr && (
        <>
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            ...
            {visible.map((p) => (
              <button key={p.name} className="pv-dex-tile" onClick={() => loadDetail(p.name)}>
                <img
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${dexId(p)}.png`;
                  }}
                />
                <div className="capitalize text-xs font-semibold mt-1 truncate">
                  #{dexId(p)} {p.name}
                </div>
              </button>
            ))}
      ...
      {selected && !detailLoading && (
        <div className="mt-6">
          <button className="pv-back" onClick={() => setSelected(null)}>
            ← Back to results
          </button>
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <div className="flex flex-col items-center gap-3">
                <img
                  src={
                    selected.artwork ??
                    `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${selected.id}.png`
                  }
                  alt={selected.name}
                ...
                {selected.cries && (
                  <button className="pv-ksave" onClick={playCry}>
                    ▶ Play Cry
                  </button>
                )}
              </div>
              <div className="flex-1">
                ...
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {selected.stats.map((s) => (
                    <div key={s.name} className="pv-stat-box">
                      <div className="text-xs capitalize" style={{ color: "var(--t3)" }}>
                        {s.name}
                      </div>
                ...
                <div className="mt-4">
                  <div className="text-xs font-semibold mb-1" style={{ color: "var(--t3)" }}>
                    Abilities
                  </div>
