...
export function AdventureView() {
  ...
  const [playerParty, setPlayerParty] = useState<BattlePokemon[]>([
    {
      ...
      types: ["Electric"],
      moves: [
        { name: "Thunderbolt", type: "Electric", power: 90, pp: 15, maxPp: 15, description: "Powerful electric strike." },
        { name: "Iron Tail", type: "Normal", power: 75, pp: 15, maxPp: 15, description: "Hard tail slam." },
        { name: "Volt Tackle", type: "Electric", power: 120, pp: 5, maxPp: 5, description: "High-voltage charge." },
        { name: "Quick Attack", type: "Normal", power: 40, pp: 30, maxPp: 30, description: "Always strikes first." },
      ],
    },
    {
      ...
      types: ["Fire", "Flying"],
      moves: [
        { name: "Flamethrower", type: "Fire", power: 90, pp: 15, maxPp: 15, description: "Scorching flame blast." },
        { name: "Dragon Claw", type: "Dragon", power: 80, pp: 15, maxPp: 15, description: "Slashes with sharp claws." },
        { name: "Fire Blast", type: "Fire", power: 110, pp: 5, maxPp: 5, description: "Intense five-pronged fireball." },
        { name: "Wing Attack", type: "Flying", power: 60, pp: 25, maxPp: 25, description: "Strikes with wings." },
      ],
    },
  ]);
  ...
  const startHdBattle = (foeName: string, foeSpecies: string, level: number, types: string[]) => {
    const foe: BattlePokemon = {
      ...
      types,
      moves: [
        { name: "Tackle", type: "Normal", power: 40, pp: 35, maxPp: 35, description: "Full body charge." },
        { name: types[0] === "Fire" ? "Ember" : types[0] === "Water" ? "Water Gun" : "Quick Strike", type: types[0] || "Normal", power: 65, pp: 20, maxPp: 20, description: "Signature elemental strike." },
      ],
    };
            ...
            <>
              ...
              <AdventureWorldMap
                ...
                onSelectCreature={(creature, mode) => {
                  if (mode === "battle") {
                    startHdBattle(creature.species, creature.species, creature.level, creature.types);
                  } else {
              ...
              {nearbyOpen && (
                <AdventureNearbyDrawer
                  ...
                  onSelectBattleArena={(a) => {
                    startHdBattle(`Leader ${a.championSpecies}`, a.championSpecies, a.championLevel, ["Fire"]);
                  }}
                />
              )}
            </>
