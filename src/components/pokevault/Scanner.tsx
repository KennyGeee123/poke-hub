...
export function ScannerView({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  if (scanLocked) {
    return (
      <Paywall reason="You've used your 10 free scans. Upgrade to keep scanning unlimited cards." />
    );
  }
  return (
    <ScannerInner
...
function ScannerInner({
  ...
  async function startCamera() {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setError(
        "Camera API unavailable. Open the published app on a phone/desktop with camera access (the in-editor preview blocks getUserMedia).",
      );
      return;
    }
    ...
    try {
      ...
      if (v) {
        ...
        try {
          ...
          /* autoplay policy — user gesture already happened */
        }
      }
    } catch (e: any) {
      const name = e?.name || "";
  ...
  async function runSearch(hint?: Identified) {
    ...
    try {
      ...
      const core =
        cleaned.replace(/\s+(ex|EX|V|VMAX|VSTAR|GX|BREAK|LV\.?X)$/i, "").trim() || cleaned;
      const quote = (s: string) => `"${s.replace(/"/g, "")}"`;

      const stHint = (hint?.supertype || "").toLowerCase();
      const supertype = stHint.startsWith("trainer")
        ? "Trainer"
        : stHint.startsWith("energy")
          ? "Energy"
          : stHint.startsWith("pok")
            ? "Pokémon"
            : null;

      // Progressive query fallback: most specific → broadest
      const queries: string[] = [];
      if (supertype && hint?.hp)
        queries.push(`name:${quote(core)} hp:${hint.hp} supertype:"${supertype}"`);
      ...
      <div className="pv-scan-stage">
        {phase === "idle" && (
          <div className="pv-scan-idle">
            ...
            <div className="pv-scan-cta-sub">
              We'll use your device camera to capture a card, then match it against the live Pokémon
              TCG database.
            </div>
            <button className="pv-scan-go" onClick={startCamera}>
              📷 Activate Scanner
            </button>
              ...
              <div className="pv-scan-id">
                ...
                {ident ? (
                  <div className="pv-scan-id-sub">
                    ...
                    {typeof ident.confidence === "number"
                      ? ` · ${Math.round(ident.confidence * 100)}% conf`
                      : ""}
                  </div>
                ) : (
                  <div className="pv-scan-id-sub">
                    AI vision will read the card. Edit the name if it's wrong.
                  </div>
                )}
                <div className="pv-scan-search">
                  ...
                  />
                  <button
                    className="pv-scan-find"
                    onClick={identify}
                    disabled={!query.trim() || phase === "searching" || phase === "identifying"}
                  >
                    {phase === "searching"
                      ? "Searching…"
                      : phase === "identifying"
                ...
                <div className="pv-scan-row">
                  <button className="pv-scan-retake" onClick={retake}>
                    ↺ Retake
                  </button>
                  <button
                    className="pv-scan-retake"
                    onClick={() => {
                      setSnap(null);
                      setPhase("idle");
                      setResults([]);
        ...
        {phase === "results" && (
          <div className="pv-scan-matches">
            <div className="pv-scan-matches-hd">
              <span>
                🎴 {results.length} possible {results.length === 1 ? "match" : "matches"}
              </span>
            </div>
            {results.length === 0 ? (
