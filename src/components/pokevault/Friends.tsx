...
export function FriendsView({ onOpenCard }: { onOpenCard: (id: string) => void }) {
    ...
    return (
      <div style={{ padding: 24, maxWidth: 520, margin: "0 auto" }}>
        <div className="pv-empty">
          ...
          <div>
            Friends, vault peek, and PvP need an account. The rest of the beta works as a guest.
          </div>
          <button
  ...
  if (view.kind === "battle") {
    return (
      <PvPBattle friend={view.friend} myMon={view.myMon} onExit={() => setView({ kind: "list" })} />
    );
  }
    ...
    >
      <div className="pv-friend-card">
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          />
          <button className="pv-friend-btn" onClick={doSearch}>
            Search
          </button>
        </div>
        ...
        {search && (
          <div className="pv-friend-row" style={{ marginTop: 8 }}>
            ...
            <button className="pv-friend-btn" onClick={doSend}>
              Send Request
            </button>
          </div>
        )}
        ...
        <Section title={`🔔 Incoming (${incoming.length})`}>
          {incoming.map((f) => (
            <div className="pv-friend-row" key={f.friendship.id}>
              <Avatar
                name={f.other?.display_name || f.other?.email || "?"}
              ...
              <button
                className="pv-friend-btn"
                onClick={async () => {
                  await respondFriendRequest(f.friendship.id, true);
                  reload();
                }}
              >
                Accept
              ...
              <button
                className="pv-friend-btn pv-friend-btn-x"
                onClick={async () => {
                  await respondFriendRequest(f.friendship.id, false);
                  reload();
                }}
              >
                Decline
          ...
          {outgoing.map((f) => (
            <div className="pv-friend-row" key={f.friendship.id}>
              ...
              <button
                className="pv-friend-btn pv-friend-btn-x"
                onClick={async () => {
                  await removeFriend(f.friendship.id);
                  reload();
                }}
              >
                Cancel
        ...
        {accepted.length === 0 ? (
          ...
          accepted.map((f) => (
            <div className="pv-friend-row" key={f.friendship.id}>
              ...
              <button
                className="pv-friend-btn"
                onClick={() => setView({ kind: "vault", friend: f })}
              >
                🔒 Vault
              </button>
              <button
                className="pv-friend-btn pv-friend-btn-fight"
                onClick={() => setView({ kind: "challenge", friend: f })}
              >
                ⚔ PvP
              </button>
              <button
...
function FriendVaultView({
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    fetchFriendVault(friend.other!.user_id)
      .then((s) => {
        setSnap(s);
        setLoading(false);
      })
  ...
  return (
    <div style={{ padding: 14, maxWidth: 1100, margin: "0 auto" }}>
      <button className="pv-friend-btn" onClick={onBack}>
        ← Back
      </button>
      <div style={{ marginTop: 14, marginBottom: 10 }}>
        <div style={{ fontSize: 20, fontWeight: 800 }}>
          {friend.other?.display_name || "Trainer"}'s Vault
        </div>
        {snap ? (
        ...
        <div className="pv-friend-card">
          ...
          {theirParty.length === 0 ? (
            <div style={{ fontSize: 12, color: "var(--t3)" }}>
              They haven't trained any Pokémon yet.
            </div>
          ) : (
          ...
          {party.length === 0 ? (
            ...
            <div className="pv-pvp-team">
              {party.map((m) => (
                <button
                  key={m.id}
                  className="pv-pvp-mon pv-pvp-mon-pick"
                  onClick={() => onStart(m)}
                >
                  <img src={spriteFor(m.name)} alt={m.name} />
                  <div className="pv-pvp-mn">{m.name}</div>
                  <div className="pv-pvp-mlv">
...
function PvPBattle({
  ...
  return (
    <div style={{ padding: 14, maxWidth: 760, margin: "0 auto" }}>
      <button className="pv-friend-btn" onClick={onExit}>
        ← Exit Battle
      </button>
      <div className="pv-pvp-arena">
        <div className="pv-pvp-side">
          ...
          {foeMon ? (
            <>
              <img
                className="pv-pvp-actor pv-pvp-actor-foe"
                src={spriteFor(foeMon.name)}
                alt={foeMon.name}
              />
              <div className="pv-pvp-stat">
                <div style={{ fontWeight: 800 }}>
                  {foeMon.name} Lv {foeMon.level}
                </div>
                <div className="pv-pvp-bar">
                  <span style={{ width: `${(foeHp / foeMon.max_hp) * 100}%` }} />
                </div>
                <div style={{ fontSize: 10, color: "var(--t3)" }}>
                  HP {foeHp}/{foeMon.max_hp}
                </div>
              </div>
            </>
          ) : (
        ...
        <div className="pv-pvp-side">
          ...
          <div className="pv-pvp-stat">
            <div style={{ fontWeight: 800 }}>
              {myMon.name} Lv {myMon.level}
            </div>
            <div className="pv-pvp-bar">
              <span style={{ width: `${(myHp / myMon.max_hp) * 100}%` }} />
            </div>
            <div style={{ fontSize: 10, color: "var(--t3)" }}>
              HP {myHp}/{myMon.max_hp}
            </div>
          </div>
        </div>
      </div>
      ...
      {over ? (
        <div
          className="pv-pvp-result"
          style={{ color: over === "win" ? "var(--gold)" : "#ef4444" }}
        >
        ...
        <div className="pv-pvp-moves">
          {myMon.attacks.slice(0, 4).map((m, i) => (
            <button
              key={i}
              className="pv-pvp-move"
              disabled={!myTurn || !foeMon}
              onClick={() => attack(m)}
            >
          ...
          {!foeMon && (
            ...
            >
              Share the lobby — your friend must open PvP with you too.
            </div>
          )}
        </div>
      )}
